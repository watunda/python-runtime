...
RUNTIME_PREFIX_REGEX = re.compile(r"""(?x)
    ^
    (
    ...
    $
""")
RUNTIME_SUFFIX_REGEX = re.compile(r"""(?x)
    ^
    [a-zA-Z0-9_.-/]*      # Empty suffix allowed
    (
...
AppConfig = collections.namedtuple(
    'AppConfig',
    'base_runtime_image dockerfile_python_version entrypoint has_requirements_txt'
)
...
def get_app_config(raw_config, args):
    ...
    base_runtime_image = '{}python{}'.format(args.runtime_prefix,
                                             args.runtime_suffix)
...
def generate_files(app_config):
    ...
    dockerfile = DOCKERFILE_TEMPLATE.format(
        base_runtime_image=app_config.base_runtime_image,
...
def gen_dockerfile(args):
    ...
    with open(args.config, 'r', encoding='utf8') as yaml_config_file:
...
def parse_args(argv):
    ...
    parser.add_argument(
        '--runtime-prefix',
        type=functools.partial(
            validation_utils.validate_arg_regex, flag_regex=RUNTIME_PREFIX_REGEX),
        help='String to prepend to Docker image name')
    parser.add_argument(
        '--runtime-suffix',
        type=functools.partial(
            validation_utils.validate_arg_regex, flag_regex=RUNTIME_SUFFIX_REGEX),
        help='String to append to Docker image name')
    parser.add_argument(
        '--source-dir',
        type=functools.partial(
            validation_utils.validate_arg_regex, flag_regex=PRINTABLE_REGEX),
